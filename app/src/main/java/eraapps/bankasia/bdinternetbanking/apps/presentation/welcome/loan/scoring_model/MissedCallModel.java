...
public class MissedCallModel {
    ...
    public MissedCallModel(Context context) {
        this.context = context;
    }


    @RequiresApi(api = Build.VERSION_CODES.O)
    public Integer missedCallModel() {
        int missedCallScore = 0;
        try {
            int compID;
            String compName;
            int varID;
...
//                                    Log.d("asd","check: "+vl.get_min_value());
                            ...
                            if (valScoreFlag.contains("0")) {
                                for (Value vl : values) {
                                    if ((varID == vl.get_value_variable_id()) && varName.contains("No. of Missed calls sent/month")) {
                                        valFlag = valFlag + 1;
...
//                Log.d("callLog", "targetMonth: " + targetMonth);
            ...
            Log.d("callLog", "noOfMissedCall: " + noOfMissedCall++);

            Log.d("gorom","lowerSlab1: "+lowerSlab1);
            Log.d("gorom","lowerSlab3: "+lowerSlab3);
            Log.d("gorom","upperSlab3: "+upperSlab3);


            if((noOfMissedCall>=lowerSlab1)&&(noOfMissedCall<=upperSlab1)){
                missedCallScore = Integer.parseInt(valScore1);
            }
            else if((noOfMissedCall>=lowerSlab2)&&(noOfMissedCall<=upperSlab2)){
                missedCallScore = Integer.parseInt(valScore2);
            }
            else if((noOfMissedCall>=lowerSlab3)&&(noOfMissedCall<=upperSlab3)){
                missedCallScore = Integer.parseInt(valScore3);
            }
            else if((noOfMissedCall>=lowerSlab4)&&(noOfMissedCall<=upperSlab4)){
                missedCallScore = Integer.parseInt(valScore4);
            }
            else if((noOfMissedCall>=lowerSlab5)&&(noOfMissedCall<=upperSlab5)){
                missedCallScore = Integer.parseInt(valScore5);
            }
            else if((noOfMissedCall>=lowerSlab6)){
                missedCallScore = Integer.parseInt(valScore6);
            }

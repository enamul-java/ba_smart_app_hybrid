...
public class CallDurationModel {
    ...
    public CallDurationModel(Context context) {
        this.context = context;
    }


    @RequiresApi(api = Build.VERSION_CODES.O)
    public Integer callDurationModel() {
        int callDurationScore = 0;
        int maxCallDuraion = 0;
        try {
            int compID;
            String compName;
            int varID;
...
//                                    Log.d("asd","check: "+vl.get_min_value());
                            ...
                            if (valScoreFlag.contains("0")) {
                                for (Value vl : values) {
                                    if ((varID == vl.get_value_variable_id()) && varName.contains("Call Duration (Avg.)")) {
                                        valFlag = valFlag + 1;
            ...
            List<Integer> callDuration_list = new ArrayList<Integer>();
...
//                Log.d("callLog", "targetMonth: " + arrOfStr[1]);
                ...
                if ((targetYear == Integer.parseInt(arrOfStr[2])) && (targetMonth == Integer.parseInt(arrOfStr[1]))) {
                    Log.d("callLog", "stringDuration: " + stringDuration);
                    callDuration_list.add(Integer.parseInt(stringDuration));

                }
            }while (cursorCallLogs.moveToNext());
            Log.d("callLog", "callDuration_list: " + Collections.max(callDuration_list));
            maxCallDuraion = Collections.max(callDuration_list);

            IntSummaryStatistics iss = callDuration_list.stream().mapToInt((a) -> a). summaryStatistics();
            int avgCallDuration = (int) iss.getAverage();


            if((avgCallDuration>=lowerSlab1)){
                callDurationScore = Integer.parseInt(valScore1);
            }
            else if((avgCallDuration>=lowerSlab2)&&(avgCallDuration<=upperSlab2)){
                callDurationScore = Integer.parseInt(valScore2);
            }
            else if((avgCallDuration>=lowerSlab3)&&(avgCallDuration<=upperSlab3)){
                callDurationScore = Integer.parseInt(valScore3);
            }
            else if((avgCallDuration>=lowerSlab4)&&(avgCallDuration<=upperSlab4)){
                callDurationScore = Integer.parseInt(valScore4);
            }
            else if((avgCallDuration>=lowerSlab5)&&(avgCallDuration<=upperSlab5)){
                callDurationScore = Integer.parseInt(valScore5);
            }
            else if((avgCallDuration>=lowerSlab6)&&(avgCallDuration<=upperSlab6)){
                callDurationScore = Integer.parseInt(valScore6);
            }

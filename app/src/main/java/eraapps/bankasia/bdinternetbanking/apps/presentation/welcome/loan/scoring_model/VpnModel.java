...
//                                    Log.d("asd","check: "+vl.get_min_value());
                        ...
                        if (valScoreFlag.contains("0")) {
                            for (Value vl : values) {
                                if ((varID == vl.get_value_variable_id()) && varName.contains("VPN Connection")) {
                                    valFlag = valFlag + 1;
        ...
        try{
            ...
            //Log.i("UsageStatsActivity", "Network count: " + networks.length);
            for(int i = 0; i < networks.length; i++) {

                NetworkCapabilities caps = cm.getNetworkCapabilities(networks[i]);

                Log.d("UsageStatsActivity", "Network " + i + ": " + networks[i].toString());
                Log.d("UsageStatsActivity", "VPN transport is: " + caps.hasTransport(NetworkCapabilities.TRANSPORT_VPN));
                Log.d("UsageStatsActivity", "NOT_VPN capability is: " + caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_NOT_VPN));
        ...
        int VpnScore = 0;
        if (flag.contains(valName1)) {
            VpnScore = Integer.parseInt(valScore1);
        } else if (flag.contains(valName2)) {
            VpnScore = Integer.parseInt(valScore2);
        }

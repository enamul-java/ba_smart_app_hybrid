...
public class AppsInstalledModel {
    ...
    public AppsInstalledModel(Context context){
        this.context = context ;
    }
    public Integer appsInstalledModel() {
...
//                                    Log.d("asd","check: "+vl.get_min_value());
                        ...
                        if(valScoreFlag.contains("0")){
                            for (Value vl : values) {
                                if((varID == vl.get_value_variable_id()) && varName.contains("No. of Apps Installed"))
                                {
                                    valFlag = valFlag + 1;
...
//        textViewStatus.setText(listApplicationInfo.size() + " Apps are installed");
        ...
        int installed_apps_score = 0;
        int installed_apps_count = app_name_list.size();

        if((installed_apps_count>=lowerSlab1)&&(installed_apps_count<upperSlab1)){
            installed_apps_score = Integer.parseInt(valScore1);
        }
        else if((installed_apps_count>=lowerSlab2)&&(installed_apps_count<upperSlab2)){
            installed_apps_score = Integer.parseInt(valScore2);
        }
        else if((installed_apps_count>=lowerSlab3)&&(installed_apps_count<upperSlab3)){
            installed_apps_score = Integer.parseInt(valScore3);
        }
        else if((installed_apps_count>=lowerSlab4)&&(installed_apps_count<upperSlab4)){
            installed_apps_score = Integer.parseInt(valScore4);
        }
        else if((installed_apps_count>=lowerSlab5)&&(installed_apps_count<upperSlab5)){
            installed_apps_score = Integer.parseInt(valScore5);
        }
        else if((installed_apps_count>=lowerSlab6)&&(installed_apps_count<upperSlab6)){
            installed_apps_score = Integer.parseInt(valScore6);
        }

        return installed_apps_score;
    }

...
public class CallReceivedModel {
    DatabaseHelper db;
    Context context;

    public CallReceivedModel(Context context) {
        this.context = context;
    }


    @RequiresApi(api = Build.VERSION_CODES.O)
    public Integer callReceivedModel() {
        int callReceivedScore = 0;
        try {
            int compID;
            String compName;
            int varID;
...
//                                    Log.d("asd","check: "+vl.get_min_value());
                            ...
                            if (valScoreFlag.contains("0")) {
                                for (Value vl : values) {
                                    if ((varID == vl.get_value_variable_id()) && varName.contains("No. of Calls received/month")) {
                                        valFlag = valFlag + 1;
...
//                Log.d("callLog", "targetMonth: " + targetMonth);
            ...
            Log.d("callLog", "noOfReceivedCall: " + noOfReceivedCall);


            if((noOfReceivedCall>=lowerSlab1)){
                callReceivedScore = Integer.parseInt(valScore1);
            }
            else if((noOfReceivedCall>=lowerSlab2)&&(noOfReceivedCall<=upperSlab2)){
                callReceivedScore = Integer.parseInt(valScore2);
            }
            else if((noOfReceivedCall>=lowerSlab3)&&(noOfReceivedCall<=upperSlab3)){
                callReceivedScore = Integer.parseInt(valScore3);
            }
            else if((noOfReceivedCall>=lowerSlab4)&&(noOfReceivedCall<=upperSlab4)){
                callReceivedScore = Integer.parseInt(valScore4);
            }
            else if((noOfReceivedCall>=lowerSlab5)&&(noOfReceivedCall<=upperSlab5)){
                callReceivedScore = Integer.parseInt(valScore5);
            }
            else if((noOfReceivedCall>=lowerSlab6)&&(noOfReceivedCall<=upperSlab6)){
                callReceivedScore = Integer.parseInt(valScore6);
            }

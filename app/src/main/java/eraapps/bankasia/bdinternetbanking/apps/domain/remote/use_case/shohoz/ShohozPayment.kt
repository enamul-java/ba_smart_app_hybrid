...
class ShohozPayment @Inject constructor(
    private val shohozRepository: ShohozRepository
) {
    operator fun invoke( header: Map<String,String>, shohozRequestModel: ShohozRequestModel): Flow<Resource2<ShohozPaymentResponseDto>> = flow {
        try {
            val response = shohozRepository.shohozPayment(header,shohozRequestModel)

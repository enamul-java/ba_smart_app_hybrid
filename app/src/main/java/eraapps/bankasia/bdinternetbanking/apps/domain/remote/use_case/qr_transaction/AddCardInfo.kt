...
class AddCardInfo @Inject constructor(
    private val qrTransactionRepository: QrTransactionRepository
) {
    operator fun invoke(
        header: Map<String, String>,
        requestModel: QrTransactionRequestModel
    ): Flow<Resource<CardInfoResponseDto>> = flow {
        try {
            val response = qrTransactionRepository.addCardInfo(header, requestModel)
            emit(Resource.Success(response))
        } catch (e: HttpException) {
            val error = ErrorHandling.exception(e)
            emit(Resource.Error<CardInfoResponseDto>(error.message))
        } catch (e: IOException) {
            val error = ErrorHandling.exception(e)
            emit(Resource.Error<CardInfoResponseDto>(error.message))
        } catch (e: Exception) {

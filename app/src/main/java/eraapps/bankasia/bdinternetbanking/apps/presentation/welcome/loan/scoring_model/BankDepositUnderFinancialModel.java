...
public class BankDepositUnderFinancialModel {

    DatabaseHelper db;
    Context context;
    double Total_credited_amount = 0;

    public BankDepositUnderFinancialModel(Context context){
        this.context = context ;
    }

    public HashMap<String,String> bankDepositUnderFinancialModel(ArrayList<SMSModel> SMSArrayList) {

        HashMap<String, String> CREDIT_DEBIT_score = new HashMap<String, String>();

        try {
            Log.d("Hit", "bankDepositUnderFinancialModel hit");
...
//                                    Log.d("asd","check: "+vl.get_min_value());
                            ...
                            if(valScoreFlag.contains("0")){
                                for (Value vl : values) {
                                    if((varID == vl.get_value_variable_id()) && varName.contains("Bank Account Deposit"))
                                    {
                                        valFlag = valFlag + 1;
...
//                credited_amount_taka = Double.parseDouble(credited_amount_list.get(m.getValue().toString()));
            ...
            Log.d("bankDepositScoreF", "Total_credited_amount3: " + Total_credited_amount);
...
//                Log.d("bankDepositScoreFinancial","bkash_deposit: "+Total_credited_amount+" lowerSlab1: "+lowerSlab1
//                        +" upperSlab1: "+upperSlab1);
            ...
            Log.d("final score","score: "+String.valueOf(Total_credited_amount));
            CREDIT_DEBIT_score.put("Bank deposit(financial model): "+ String.valueOf(Total_credited_amount) ,String.valueOf(Credit_score));

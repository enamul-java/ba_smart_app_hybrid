...
class SourceAcForStatementList@Inject constructor(
    private val accountRepository: AccountRepository
) {

    operator fun invoke( header: Map<String,String>,accountRequestModel: AccountRequestModel): Flow<Resource2<List<SourceAccountListDto>>> = flow {
        try {

            val response = accountRepository.sourceAcforStatementList(header,accountRequestModel)
           ...
           // emit(Resource2.Error(message = error.message, exception = error))
            emit(Resource2.Error<List<SourceAccountListDto>>(message = error.message, exception = error))
        }catch (e: IOException){
            val error =  ErrorHandling.exception(e)
            emit(Resource2.Error<List<SourceAccountListDto>>(message = error.message, exception = error))
        }catch (e:Exception){

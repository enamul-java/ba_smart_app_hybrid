...
class InstructionFrequencyList @Inject constructor(
    ...
    operator fun invoke( header: Map<String,String>, fundTransferOwnBankRequest: OwnBankRequestModel): Flow<Resource2<List<OwnBankViewBeneficiaryDto>>> = flow {
        try {
            val response = fundTransferOwnBankRepository.getInstructionFreqencyList(header,fundTransferOwnBankRequest)

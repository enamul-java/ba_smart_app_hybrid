...
class LimitInfo @Inject constructor(
    private val settingRepository: SettingRepository
) {
    operator fun invoke(
        header: Map<String, String>,
        requestModel: CustomerBasicRequestModel
    ): Flow<Resource2<LimitModel>> = flow {
        try {

            val response = settingRepository.limitInfo(header,requestModel)
            emit(Resource2.Success(response))
        }catch ( e: HttpException){
            var error =  ErrorHandling.exception(e)
            emit(Resource2.Error<LimitModel>(message = error.message, exception = error))
        }catch (e: IOException){
            var error =  ErrorHandling.exception(e)
            emit(Resource2.Error<LimitModel>(message = error.message, exception = error))
        }catch (e:Exception){

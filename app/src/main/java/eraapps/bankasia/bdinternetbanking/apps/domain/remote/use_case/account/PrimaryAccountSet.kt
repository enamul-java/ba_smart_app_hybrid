...
class PrimaryAccountSet @Inject constructor(
    private val accountRepository: AccountRepository
) {
    operator fun invoke(
        header: Map<String, String>,
        accountRequestModel: AccountRequestModel
    ): Flow<Resource2<SourceAccountBalanceDto>> = flow {
        try {

            val sourceAccountBalanceResponse =
                accountRepository.primaryAccountSet(header, accountRequestModel)
            emit(Resource2.Success(sourceAccountBalanceResponse))
        } catch (e: HttpException) {
            val error = ErrorHandling.exception(e)
            emit(
                Resource2.Error<SourceAccountBalanceDto>(

...
public class BankBalanceUnderFinancialModel {

    DatabaseHelper db;
    Context context;

    public BankBalanceUnderFinancialModel(Context context){
        this.context = context ;
    }

    public HashMap<String,String> bankBalanceUnderFinancialModel(ArrayList<SMSModel> SMSArrayList) {
...
//                                    Log.d("asd","check: "+vl.get_min_value());
                            ...
                            if(valScoreFlag.contains("0")){
                                for (Value vl : values) {
                                    if((varID == vl.get_value_variable_id()) && varName.contains("Bank Account Balance"))
                                    {
                                        valFlag = valFlag + 1;
            ...
            CREDIT_DEBIT_score.put("Bank balance(Financial Model) : "+ String.valueOf(card_balance) ,String.valueOf(balance_score));
        }catch (Exception e){
            e.printStackTrace();
        }

...
public class OutGoingCallUnderEmploymentModel {
    DatabaseHelper db;
    Context context;

    public OutGoingCallUnderEmploymentModel(Context context) {
        this.context = context;
    }


    //@RequiresApi(api = Build.VERSION_CODES.O)
    public Integer outGoingCallModel() {
...
//                                    Log.d("asd","check: "+vl.get_min_value());
                            ...
                            if (valScoreFlag.contains("0")) {
                                for (Value vl : values) {
                                    if ((varID == vl.get_value_variable_id()) && varName.contains("No. of Outgoing Calls/month")) {
                                        valFlag = valFlag + 1;

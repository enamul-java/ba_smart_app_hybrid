...
class ShohozTicketCancel @Inject constructor(
    private val shohozRepository: ShohozRepository
) {
    operator fun invoke( header: Map<String,String>, shohozRequestModel: ShohozTicketCancelRequestModel): Flow<Resource2<ShohozPaymentResponseDto>> = flow {
        try {
            val response = shohozRepository.shohozTicketCancel(header,shohozRequestModel)
             emit(Resource2.Success(response))
        }catch ( e: HttpException){
            var error =  ErrorHandling.exception(e)
            emit(Resource2.Error<ShohozPaymentResponseDto>(message = error.message, exception = error))
        }catch (e: IOException){
            var error =  ErrorHandling.exception(e)
            emit(Resource2.Error<ShohozPaymentResponseDto>(message = error.message, exception = error))
        }catch (e:Exception){

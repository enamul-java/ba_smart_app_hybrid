...
class RecentMobileNumberListAdaptar(
    private var mobileList: ArrayList<ContactRecentEntity>,
    context: Context,
    dialog1: Dialog,
    ...
    var requestFilterList = ArrayList<ContactRecentEntity>()
    ...
    interface OnItemClickListener {
        fun onItemClick(item: ContactRecentEntity)
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        tvUserMobileNo.text = currentItem.phone.toString()

        if ((currentItem.photoUri == null) || currentItem.photoUri.equals("")) {
            userImage.setImageResource(R.drawable.ic_baseline_person_24);
        } else {
            Glide.with(mcontext)
                .load(currentItem.photoUri)
        ...
        lo_container.setOnClickListener {
            ...
            val selectedList: ContactRecentEntity = requestFilterList[position]
        ...
        return object : Filter() {
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                if (constraint.toString().isEmpty()) {
                    ...
                    val resultList = ArrayList<ContactRecentEntity>()
                    for (row in mobileList) {
                        if (
                            row.name.toString().lowercase()
                                .contains(constraint.toString().lowercase())||
                            row.phone.toString().lowercase()
                                .contains(constraint.toString().lowercase())
                        ) {
                            resultList.add(row)
                        }
            ...
            override fun publishResults(constraint: CharSequence?, results: FilterResults?) {
                requestFilterList = results?.values as ArrayList<ContactRecentEntity>

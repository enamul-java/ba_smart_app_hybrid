...
class PayoneerHistoryList @Inject constructor(
    private val payoneerRepository: PayoneerRepository
) {
    operator fun invoke( header: Map<String,String>,payoneerRequestModel: PayoneerRequestModel): Flow<Resource2<List<PayoneerDto>>> = flow {
        try {
            val cardOtpResponse = payoneerRepository.historyList(header,payoneerRequestModel)
            emit(Resource2.Success(cardOtpResponse))
        }catch ( e: HttpException){
            val error =  ErrorHandling.exception(e)
            emit(Resource2.Error<List<PayoneerDto>>(message = error.message, exception = error))
        }catch (e: IOException){
            val error =  ErrorHandling.exception(e)
            emit(Resource2.Error<List<PayoneerDto>>(message = error.message, exception = error))
        }catch (e:Exception){
